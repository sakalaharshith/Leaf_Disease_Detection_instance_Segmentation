...
def log(text, array=None):
    ...
    print(text)
def call_back():   # callback loss value
    a, b, c = return_value(epoch_list, tra_loss_list, val_loss_list)
    return a, b, c
class BatchNormaiii(KL.BatchNormalization):
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
              custom_callbacks,augmentation,  no_augmentation_sources=None):
        """Train the model.
...
class MeanAveragePrecisionCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch > 2 and (epoch+1)%self.calculate_map_at_every_X_epoch == 0:
            ...
            if logs is not None:
                ...
                logs["val_intersection_over_union"]=iou
            self._verbose_print("mAP at epoch {0} is: {1}, intersection over union is {2}".format(epoch+1, mAP,iou))
    ...
    def _calculate_mean_average_precision(self):
        ...
        for image_id in self.dataset_image_ids[:self.dataset_limit]:
            ...
            iou=utils.compute_overlaps_masks(r['masks'],gt_mask)
            iou=np.sum(iou)
